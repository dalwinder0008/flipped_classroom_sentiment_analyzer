...
import { Star, Send, Loader2, CheckCircle2, Copy, RefreshCw, BookOpen } from "lucide-react";
...
export default function SubmitReview() {
  ...
  const [formData, setFormData] = useState({
    student_name: profile?.display_name || user?.displayName || "",
    email: user?.email || "",
    course_name: "",
    rating: 5,
    content: ""
  ...
  useEffect(() => {
    if (user) {
      setFormData(prev => ({
        ...prev,
        student_name: prev.student_name || profile?.display_name || user.displayName || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const reviewData = {
        id: reviewId,
        student_name: formData.student_name || user.displayName || user.email?.split('@')[0] || "Student",
            ...
            >
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-600/50 transition-all"
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3 sm:gap-4">
                  {(Object.keys(ratingEmojis) as unknown as (keyof typeof ratingEmojis)[]).map((num) => (
                    <button
                      key={num}
                      type="button"
                      onClick={() => setFormData({ ...formData, rating: Number(num) })}
                      className={cn(
                        "flex-1 min-w-[60px] h-16 rounded-xl flex flex-col items-center justify-center transition-all gap-1",
                        formData.rating === Number(num) 
                          ? "bg-brand-600 text-white shadow-lg shadow-brand-600/20 scale-105" 
                          : "bg-white/5 text-slate-500 hover:bg-white/10"
                      )}
                    >
                      <span className="text-2xl">{ratingEmojis[num].emoji}</span>
                      <span className="text-[10px] font-bold uppercase tracking-wider">{ratingEmojis[num].label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 max-w-sm mx-auto">
                ...
                {!limitReached && (
                  <button 
                    onClick={() => {
                      setStep(1);
                      setFormData({ student_name: user?.displayName || "", email: user?.email || "", course_name: "", rating: 5, content: "" });
                      setResult(null);
                    }}
